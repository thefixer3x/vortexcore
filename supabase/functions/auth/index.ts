  ...
  try {
    ...
    switch(action){
      case 'setup-2fa':
        ...
        if (!email) {
          return new Response(JSON.stringify({
            error: 'Email is required for 2FA setup'
          }), {
            headers: {
              ...corsHeaders,
      ...
      case 'verify-2fa':
        ...
        if (!code || !factorId) {
          return new Response(JSON.stringify({
            error: 'Code and factor ID are required for 2FA verification'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      case 'get-auth-settings':
        ...
        const { data: settings, error: settingsError } = await supabase.from('auth_settings').select('*').single();
        if (settingsError) {
